...
3. **Uncommitted work is captured as a git commit object pointed to by a ref**
   `refs/open-agents/preserved/<session-id>`. Reject the user's original
...
5. **Do not weaken the existing dirty-worktree refusal** used by interactive
   `open-agents session kill` / `open-agents cleanup`. Add a separate `ForceDestroy` that the
...
- App state resolves under `~/.open-agents` only (`OPEN_AGENTS_DATA_DIR`/`OPEN_AGENTS_RUN_FILE`
...
- `StashUncommitted(ctx, info) (ref string, err error)`: build the preserve
  ...
  point `refs/open-agents/preserved/<id>` at it via `git update-ref`, return the ref name
...
- Expose the manager (or a minimal `LifecycleSaver`/`LifecycleRestorer` seam)
  ...
  **Check:** Manual run documented in report — spawn a session, edit a tracked
  ...
  `refs/open-agents/preserved/<id>` exists; restart daemon; assert worktree re-created and
