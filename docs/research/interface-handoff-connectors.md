...
The two review blockers have the same architectural cause: Open Agents is asking a rendered terminal frame to stand in for provider state and provider history. Other implementations keep these separate.
...
| Output visible before a switch is absent after it | Do not commit the switch until the source reaches a provider completion/checkpoint, the target replays through that checkpoint, and Open Agents has applied that replay idempotently. |
...
Open Agents's native TUI inspector already has independent work and composer facts, but
...
This matches Open Agents's existing chat boundary: `ResolveRequest` is already a typed action that is never derived from a message (`backend/internal/ports/chat.go:796-818`).

### 2. Output requires a commit-and-replay barrier

Open Agents's existing `ChatHistoryReader` contract is a strong foundation: settled events have stable `ProviderEventID` values so repeated imports are idempotent (`backend/internal/ports/chat.go:821-831`). The immediate hole is Codex history: `ReadHistory` silently skips running and queued turns (`backend/internal/adapters/chatdriver/codexappserver/history.go:102-124`). That turns “history is not settled yet” into apparent success with missing output.
...
3. **Record a checkpoint.** Persist the source's last settled provider turn/item identity and Open Agents's normalized high-water sequence. Streamed chunks are responsive previews; a completed item/message/terminal snapshot is the repair authority.
...
- **ACP capabilities:** test full `session/load`, resume-only ACP, and failed/partial load. Resume-only must not promise transcript continuity unless Open Agents's own journal already contains the authoritative snapshot.
