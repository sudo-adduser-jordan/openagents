---
name: open-agents-desktop-dev
description: Launch, restart, or troubleshoot the real Open Agents Electron desktop app from this repository; run a checkout against isolated or real local Open Agents data; combine PR branches for local UI review; and diagnose stale Electron processes, port conflicts, or preload bridge mismatches. Use whenever asked to run, open, show, or visually verify Open Agents frontend changes in the actual desktop app rather than open-agents preview, dev:web, or mock data.
---

# Open Agents Desktop Dev
...
- Use **isolated mode** by default for implementation and destructive testing. Electron uses port `3002`, `~/.open-agents/dev/running.json`, `~/.open-agents/dev/data`, and `~/.open-agents/dev/electron`.
- Use **real-data mode** only when the user explicitly asks to see this machine's actual Open Agents projects or sessions. Start the checkout's dev daemon on the isolated dev port/run file while pointing `OPEN_AGENTS_DATA_DIR` at the real Open Agents data directory. This is a separate daemon process using real data; do not describe it as the installed app's daemon.
...
- Real-data mode reads the same database as the installed app, so both windows show the same projects and sessions. Quit the packaged app first where possible: two daemons sharing one SQLite database can cross-talk, and acting on the wrong window runs unmerged code against real sessions. The dev window is marked with an `Open Agents (dev)` title, the default Electron dock icon, and an in-app `dev` badge — verify those markers before acting.
...
Use the full command paths and start times to distinguish this checkout from other Open Agents worktrees. On Windows, use `Get-CimInstance Win32_Process` and `Get-NetTCPConnection`; preserve the same exact-target rule.
...
```bash
cd frontend
env -u OPEN_AGENTS_DATA_DIR -u OPEN_AGENTS_RUN_FILE -u OPEN_AGENTS_PORT npm run dev
```
...
Remove-Item Env:OPEN_AGENTS_DATA_DIR, Env:OPEN_AGENTS_RUN_FILE, Env:OPEN_AGENTS_PORT -ErrorAction SilentlyContinue
npm run dev
```

### Real-data mode

First resolve and report the actual data directory. In an Open Agents worker session, `OPEN_AGENTS_DATA_DIR` normally already identifies it. Otherwise the repository default is the absolute path corresponding to `~/.open-agents/data`.
...
```bash
cd frontend
env -u OPEN_AGENTS_RUN_FILE -u OPEN_AGENTS_PORT OPEN_AGENTS_DATA_DIR="${OPEN_AGENTS_DATA_DIR:-$HOME/.open-agents/data}" npm run dev
```
...
```powershell
Set-Location frontend
$realOpenAgentsData = if ($env:OPEN_AGENTS_DATA_DIR) { $env:OPEN_AGENTS_DATA_DIR } else { Join-Path $HOME ".open-agents/data" }
Remove-Item Env:OPEN_AGENTS_RUN_FILE, Env:OPEN_AGENTS_PORT -ErrorAction SilentlyContinue
$env:OPEN_AGENTS_DATA_DIR = $realOpenAgentsData
npm run dev
```

Do not print unrelated environment variables: Open Agents sessions may carry credentials. It is safe to report only `OPEN_AGENTS_DATA_DIR`, `OPEN_AGENTS_RUN_FILE`, and `OPEN_AGENTS_PORT`.
...
If several Open Agents windows exist, identify the newest Electron main process belonging to this checkout. On macOS, foreground that exact PID only when needed:
...
- A renderer error such as `openAgentsBridge.<method> is not a function` almost always means the renderer hot-reloaded against a stale preload. Restart Electron before changing code to add guards around a bridge method that should exist.
...
4. Confirm the captured Electron PID and dev daemon listener are gone. Leave installed Open Agents and other worktrees untouched.

## Common mistakes

- `open-agents preview` controls the Open Agents Browser panel; it does not launch the desktop shell.
...
- Multiple dev instances share `~/.open-agents/dev/electron` by default, and Chromium's
  ...
  ```bash
  env -u OPEN_AGENTS_PORT \
    OPEN_AGENTS_DEV_ELECTRON_DIR="$HOME/.open-agents/dev/<name>/electron" \
    OPEN_AGENTS_RUN_FILE="$HOME/.open-agents/dev/<name>/running.json" \
    OPEN_AGENTS_DATA_DIR="$HOME/.open-agents/dev/<name>/data" \
    npm run dev
  ```

  Without all three the instances still collide, on the profile lock, the run
  file, or the daemon's SQLite data.
- An inherited `OPEN_AGENTS_DATA_DIR` changes dev mode from isolated data to real data. Always choose and report the mode instead of inheriting it accidentally.
