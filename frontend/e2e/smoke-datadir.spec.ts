...
// ~/.open-agents layout or the real daemon/storage. The on-disk boundary is exercised only
// in the packaged-app pod gate (#2697), which today runs a boot-level smoke (app
// launches, daemon ready), NOT the full ~/.open-agents layout assertion — that stays in
// the pod data-dir script as future per-case work.
//
// The on-disk checks (Electron userData resolves under ~/.open-agents/electron, daemon
// data under ~/.open-agents, no OS-default app-data writes, OPEN_AGENTS_DATA_DIR override) run in
...
// the ~/.open-agents filesystem assertions stay in the pod data-dir script.
