 ...
 * the OPEN_AGENTS_KEEP_DAEMON env var. When set to an explicit truthy value ("1",
 ...
 * an explicit `open-agents stop`. Any other value — including conventional falsy ones
 ...
 */
export function keepDaemonAlive(env: { OPEN_AGENTS_KEEP_DAEMON?: string }): boolean {
	const raw = env.OPEN_AGENTS_KEEP_DAEMON?.trim().toLowerCase();
 ...
 * stay unlinked even when the app is later reopened without OPEN_AGENTS_KEEP_DAEMON).
 * Only a normal app-owned daemon ("app") is linked; a keep-alive daemon
 * ("persistent") and headless `open-agents start` daemons (owner unset/empty) stay
