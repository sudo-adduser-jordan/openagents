...
describe("Open Agents action translation", () => {
...
describe("agent-browser runtime lifecycle", () => {
	...
	async function fixture(overrides: {
		...
		const dataDir = await mkdtemp(path.join(shortTempDir, "open-agents-browser-runtime-test-"));
		const bridge = {
	...
	it("uses a private explicit socket directory and a bounded namespace", async () => {
		...
		try {
			...
			if (process.platform === "win32") {
				...
				expect(socketDir).toMatch(/^\/tmp\/open-agents-br-/);
			}
			expect(namespace).toMatch(/^s-[0-9a-f]{4}-[0-9a-f]{12}$/);
	...
	it("passes only the native runtime allowlist and Open Agents-scoped variables", async () => {
	...
	it("rejects an oversized Unix socket path before starting the bridge", async () => {
		const socketBase = await mkdtemp(path.join(os.tmpdir(), "open-agents-browser-runtime-long-path-test-"));
	...
	it("recovers only confirmed-dead marked run roots", async () => {
		const dataDir = await mkdtemp(path.join(os.tmpdir(), "open-agents-browser-scavenge-test-"));
		try {
			...
			const legacy = path.join(dataDir, "open-agents-legacy-session");
			...
			await writeFile(
				deadOwnerPath,
				JSON.stringify({ marker: "OPEN_AGENTS_BROWSER_RUNTIME_V1", pid: 101, startedAt: new Date().toISOString(), token: "a".repeat(32) }),
			);
			await writeFile(
				shortDeadOwnerPath,
				JSON.stringify({ marker: "OPEN_AGENTS_BROWSER_RUNTIME_V1", pid: 101, startedAt: new Date().toISOString(), token: "e".repeat(32) }),
			);
			...
			await writeFile(
				...
				JSON.stringify({ marker: "OPEN_AGENTS_BROWSER_RUNTIME_V1", pid: 202, startedAt: new Date().toISOString(), token: "b".repeat(32) }),
			);
	...
	it.skipIf(process.platform === "win32")("removes only confirmed-dead socket aliases owned by this data root", async () => {
		const dataDir = await mkdtemp(path.join(shortTempDir, "open-agents-browser-alias-data-"));
		const aliasRoot = await mkdtemp(path.join(shortTempDir, "open-agents-browser-alias-root-"));
		const foreignRoot = await mkdtemp(path.join(shortTempDir, "open-agents-browser-alias-foreign-"));
		try {
			...
			await Promise.all([
				symlink(deadTarget, path.join(aliasRoot, "open-agents-br-101-aaaaaaaaaaaa"), "dir"),
				symlink(liveTarget, path.join(aliasRoot, "open-agents-br-202-bbbbbbbbbbbb"), "dir"),
				symlink(foreignTarget, path.join(aliasRoot, "open-agents-br-303-cccccccccccc"), "dir"),
				symlink(deadTarget, path.join(aliasRoot, "open-agents-br-not-owned"), "dir"),
			]);

			await scavengeBrowserSocketAliases(dataDir, (pid) => pid === 202, undefined, aliasRoot);

			expect((await readdir(aliasRoot)).sort()).toEqual([
				"open-agents-br-202-bbbbbbbbbbbb",
				"open-agents-br-303-cccccccccccc",
				"open-agents-br-not-owned",
			]);
		} finally {
			await Promise.all([
...
const nativeBinary = process.env.OPEN_AGENTS_AGENT_BROWSER_TEST_BINARY;
describe.skipIf(!nativeBinary)("agent-browser native compatibility", () => {
	it("connects the pinned native daemon and keeps native tab lifecycle commands aligned", async () => {
		...
		const runtime = new AgentBrowserRuntime({
			...
			dataDir: path.join(shortTempDir, "open-agents-agent-browser-native-test"),
		});
		try {
