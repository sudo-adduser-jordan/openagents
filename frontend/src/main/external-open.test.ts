...
describe("isAllowedAppExternalURL", () => {
	it("allows web and mail handoff URLs from the app renderer", () => {
		expect(isAllowedAppExternalURL("https://github.com/sudo-adduser-jordan/open-agents/issues/new")).toBe(true);
		expect(isAllowedAppExternalURL("http://localhost:5173/help")).toBe(true);
		expect(isAllowedAppExternalURL("mailto:prateek@untrivial.ai?subject=Open Agents%20feedback")).toBe(true);
	});
	...
	it("opens allowed URLs through the native shell opener", async () => {
		...
		await openAllowedAppExternalURL("mailto:prateek@untrivial.ai?subject=Open Agents%20feedback", { openExternal });

		expect(openExternal).toHaveBeenCalledWith("mailto:prateek@untrivial.ai?subject=Open Agents%20feedback");
	});
