...
const RUNTIME_OWNER_MARKER = "OPEN_AGENTS_BROWSER_RUNTIME_V1";
const RUNTIME_OWNER_FILE = "owner.json";
const RUNTIME_ROOT_PATTERN = /^(?:run-(\d+)-[0-9a-f]{12}|r-[0-9a-f]{10})$/;
const SOCKET_ALIAS_PATTERN = /^open-agents-br-(\d+)-[0-9a-f]{12}$/;
...
export class AgentBrowserRuntime {
	...
	async run(
		...
		// Open Agents session runtime remains alive. Its replacement starts streaming by
	...
	async screenshot(
		...
		try {
			...
			if (image.length > MAX_SCREENSHOT_BYTES) {
				throw runtimeError("AGENT_BROWSER_OUTPUT_TOO_LARGE", "Browser screenshot exceeded Open Agents's size limit");
			}
		...
		this.runtimeRootPromise = (async () => {
			...
			try {
				...
				if (!this.socketDirOverride && process.platform !== "win32") {
					...
					// every Unix build. Keep the real runtime state under ~/.open-agents, but
					// hand the native process a short symlink path whose length does
					// not depend on the user's home directory or username.
					socketDirAlias = path.join("/tmp", `open-agents-br-${process.pid}-${randomBytes(6).toString("hex")}`);
	...
	private environment(runtime: SessionRuntime): NodeJS.ProcessEnv {
		...
		// execution/locale essentials and add Open Agents's explicitly scoped contract below.
	...
	private async assertBinary(): Promise<void> {
		try {
			...
			throw runtimeError(
				...
				`Open Agents's browser automation component was not found at ${this.options.binaryPath}. Reinstall or rebuild the desktop app.`,
			);
		}
	}
}
...
/** Remove only confirmed-dead frontend aliases owned by this Open Agents data root. */
...
export function validateAgentBrowserArguments(args: string[]): void {
	...
	if (!ALLOWED_COMMANDS.has(command)) {
		throw runtimeError("AGENT_BROWSER_COMMAND_BLOCKED", `agent-browser command is not enabled in Open Agents: ${command}`);
	}
	for (const arg of args) {
		const lower = arg.toLowerCase();
		if (FORBIDDEN_FLAGS.some((flag) => lower === flag || lower.startsWith(`${flag}=`))) {
			throw runtimeError("AGENT_BROWSER_COMMAND_BLOCKED", `agent-browser flag is managed by Open Agents: ${arg}`);
		}
	}
	...
	if (command === "diff" && args[1]?.toLowerCase() !== "snapshot") {
		throw runtimeError("AGENT_BROWSER_COMMAND_BLOCKED", "Only snapshot diff is enabled in Open Agents");
	}
	if (command === "get" && args[1]?.toLowerCase() === "cdp-url") {
		throw runtimeError("AGENT_BROWSER_COMMAND_BLOCKED", "The private Open Agents CDP endpoint cannot be displayed");
	}
}
	...
	return new Promise((resolve, reject) => {
		...
		const append = (
			...
			if (current.length + chunk.length > MAX_OUTPUT_BYTES) {
				child.kill();
				finish(runtimeError("AGENT_BROWSER_OUTPUT_TOO_LARGE", "agent-browser output exceeded Open Agents's limit"));
				return current;
			}
...
function sessionNamespace(sessionId: string): string {
	return `s-${createHash("sha256").update(sessionId).digest("hex").slice(0, 4)}`;
}
...
function assertAgentBrowserSocketPath(socketDir: string, namespace: string, platform: NodeJS.Platform): void {
	...
	if (byteLength > AGENT_BROWSER_UNIX_SOCKET_PATH_MAX_BYTES) {
		throw runtimeError(
			"AGENT_BROWSER_START_FAILED",
			`Agent Browser socket path is ${byteLength} bytes; Unix supports at most ${AGENT_BROWSER_UNIX_SOCKET_PATH_MAX_BYTES}. Open Agents needs a shorter socket directory.`,
		);
	}
}
