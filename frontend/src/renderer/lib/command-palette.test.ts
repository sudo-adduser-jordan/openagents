	...
	return [
		{
			...
			sessions: [
				...
				session({ id: "mgr", title: "Manager session", kind: "manager" }),
			],
		},
	];
}
...
describe("buildCommands grouping", () => {
	...
	it("puts current-scoped actions in the Current group when the project is valid", () => {
		...
		expect(map.get("current-open-manager")?.group).toBe("current");
	...
	it("disables New task with a reason when the route project is absent from workspaces", () => {
		...
		expect(byId(items).has("current-open-manager")).toBe(false);
		expect(byId(items).has("current-project-settings")).toBe(false);
	});

	it("disables New task and Open manager while the project manager is restarting", () => {
		...
		expect(map.get("current-new-task")?.disabledReason).toBe("Manager restarting");
		expect(map.get("current-open-manager")?.disabled).toBe(true);
		expect(map.get("current-open-manager")?.disabledReason).toBe("Manager restarting");
		expect(map.get("current-project-settings")?.disabled).toBeFalsy();
	});

	it("omits Copy branch for a synthetic (session/<id>) branch and for managers", () => {
		...
		const managerCommands = buildCommands({ workspaces: workspaces(), currentSessionId: "mgr" });
		expect(byId(managerCommands).has("current-copy-branch")).toBe(false);
	});

	it("recognises a manager by its id suffix, not just its kind", () => {
		const rows = workspaces();
		rows[0].sessions.push(session({ id: "proj-1-manager", title: "legacy manager", branch: "main" }));
		const items = buildCommands({ workspaces: rows, currentSessionId: "proj-1-manager" });
...
describe("buildSessionActions", () => {
	...
	it("never offers Resume or Copy branch for a manager", () => {
		const items = buildSessionActions(
			workspace,
			session({ id: "proj-1-manager", kind: "manager", status: "terminated", branch: "main" }),
		);
