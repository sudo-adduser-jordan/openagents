...
/**
 * Delivery state for a message Open Agents sent. `uncertain` is deliberately not merged
 * into `failed`: the provider may have accepted the turn while Open Agents lost the
...
export interface ApprovalDetail {
	...
	method?: string;
	/** The provider decision id Open Agents successfully returned. */
	decision?: string;
...
/** ACP form/URL elicitation projected into a durable Open Agents activity. */
 ...
 * joins a turn in flight, and Open Agents's only durable write that can attach to one is the
...
/** Open Agents's permission vocabulary, applied per turn in chat mode. */
...
export interface ChatConfigChoice {
	/** Exact Open Agents permission equivalent supplied by the daemon, when supported. */
...
export interface ConversationAccount {
	...
	planLabel?: string;
	/**
	 * When the provider last demanded credentials Open Agents does not hold. Present means the
...
/**
 * The provider's thread lifecycle, which is NOT Open Agents's controller state.
 *
...
export interface ConversationSnapshot {
	 ...
	 *  restart and applies to turns Open Agents dispatches on the user's behalf. */
...
export interface ConversationBranchMaterialization {
	...
	/** True when Open Agents's bounded reconstructed context omitted some text. */
