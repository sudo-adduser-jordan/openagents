...
/**
 * Shared shortcut-binding editor. `mode: "content"` renders the inline settings
 * panel body; `mode: "dialog"` renders the same body inside the shortcut
 * settings dialog (including the conflict confirm on top of it).
 */
export function ShortcutBindingsEditor({
	...
	const surfaceActiveRef = useRef(mode === "dialog" ? open : active);
	const recordingRequestRef = useRef(0);
	const surfaceActive = mode === "dialog" ? open : active;
	...
	useEffect(() => {
		if (!surfaceActive || !recording) return;
	...
	const search = mode === "dialog" ? (
		<label className="relative">
			<Search className="pointer-events-none absolute left-3 top-1/2 size-icon-base -translate-y-1/2 text-settings-muted" aria-hidden="true" />
			<Input
				type="search"
		...
		<div className="sticky top-0 z-10 flex flex-col gap-3 bg-[var(--color-bg-settings-row)] pb-2">
			<label className="flex h-9! min-w-0 items-center gap-2 rounded-md border border-(--color-border-settings-input) bg-(--color-bg-settings-input) px-3">
				<Search className="size-4 shrink-0 text-settings-muted" aria-hidden="true" />
				<Input
					type="search"
		...
		<div className="flex flex-col gap-2">
			{filteredShortcuts.map((shortcut) => {
				...
				return (
					<div
						className={cn(
							`flex min-h-(--size-settings-row) items-center rounded-lg bg-(--color-bg-settings-row) ${mode === "dialog" ? "px-3" : "px-0"} py-3`,
	...
	const toastBlock = toast ? (
		<div
			className={cn(
				"pointer-events-auto flex items-start gap-3 rounded-xl border border-(--color-border-settings-dialog) bg-settings-dialog px-4 py-3 shadow-[var(--shadow-settings-dialog)]",
				mode === "dialog"
					? "absolute bottom-4 right-4 z-[calc(var(--z-overlay)+2)] w-[min(24rem,calc(100%-2rem))]"
					: "sticky bottom-2 z-10 ml-auto w-[min(24rem,100%)]",
			)}
			role="status"
			aria-live="polite"
			aria-atomic="true"
				...
				>
					...
					>
						...
						<div className={cn(settingsDialogBodyClass, "min-h-0 flex-1 gap-3")}>
							{search}
							{rows}
							<div className={settingsDialogFooterClass}>{footer}</div>
						</div>
	...
	return (
		<>
			<div className="relative flex flex-col gap-3">
				{search}
				{rows}
				<div className="flex items-center gap-3 pt-2">{footer}</div>
				{toastBlock}
			</div>
			{conflictDialog}
		</>
