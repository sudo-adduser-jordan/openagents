...
describe("ShellTerminalTab rename", () => {
	it("commits a new title on Enter", () => {
		...
		fireEvent.doubleClick(screen.getByRole("tab", { name: "open-agents" }));
	...
	it("discards on Escape and leaves the title unchanged", () => {
		...
		expect(screen.getByRole("tab", { name: "open-agents" })).toBeInTheDocument();
	});
	...
	it("selects the tab on single click", () => {
		...
		fireEvent.click(screen.getByRole("tab", { name: "open-agents" }));
		expect(onSelect).toHaveBeenCalled();
	});

	it("makes the full connected tile the semantic selection button", () => {
		const { onSelect } = renderTab({ appearance: "connected" });
		const tab = screen.getByRole("tab", { name: "open-agents" });
		expect(tab).toHaveClass("h-full", "min-w-0", "px-2", "cursor-pointer", "focus-visible:outline-2");
		fireEvent.click(tab);
		expect(onSelect).toHaveBeenCalledOnce();
	});

	it("reveals close in the terminal glyph's slot on hover", () => {
		renderTab({ appearance: "connected", isActive: true });

		const closeButton = screen.getByRole("button", { name: "Close terminal open-agents" });
		expect(closeButton.parentElement).toHaveClass("absolute", "inset-y-0", "left-2");
		...
		expect(screen.getByRole("tab", { name: "open-agents" }).querySelector("svg")).toHaveClass("group-hover:opacity-0");
		expect(screen.getByRole("tab", { name: "open-agents" })).toHaveAttribute("aria-selected", "true");
	});
	...
	it("uses a neutral active surface with a strong foreground selection line", () => {
		...
		expect(screen.getByRole("tab", { name: "open-agents" }).closest("[data-terminal-tab-frame]")).toHaveClass(
			"h-full",
			"self-stretch",
			"bg-overlay",
		);
		expect(screen.getByTestId("active-terminal-tab-indicator")).toHaveClass("bottom-0", "h-0.5");
		expect(screen.getByRole("tab", { name: "open-agents" }).closest("[data-terminal-tab-frame]")).not.toHaveClass("rounded-md", "before:bg-accent", "after:h-px");
	});
	...
	it("closes from the glyph slot without selecting the tab", () => {
		...
		fireEvent.click(screen.getByRole("button", { name: "Close terminal open-agents" }));
	...
	it("optically centers the auxiliary terminal glyph with its label", () => {
		...
		expect(screen.getByRole("tab", { name: "open-agents" }).querySelector("svg")?.parentElement).not.toHaveClass("-translate-y-px");
	});
});
