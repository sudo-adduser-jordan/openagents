...
function cacheDescriptor(
	...
	// The reset nonce discriminates a restored session (same id, replaced live
	// terminal) from the live one: folding it into the cache key alone
	// makes restore mount a brand-new entry while `generation` stays the raw
	// value the workspace reconcile loop compares against
	// session.terminalGeneration — so a restore does not look like a
...
export function TerminalCacheProvider({
	...
	const resolveCreateMux = useCallback((): (() => TerminalMux) => muxPool.acquire, [muxPool]);
	...
	const activate = useCallback(
		(descriptor: TerminalCacheDescriptor, props: TerminalPaneProps, slot: HTMLDivElement) => {
			...
			if (!parking) return;
			const cachedProps = { ...props, createMux: resolveCreateMux() };
...
export function TerminalPane({
	...
	// a fresh mount. Reading through the store hook is what makes the bump
	// reactive; cacheDescriptor/resolveCreateMux read the same value
	// non-reactively. Placed above the early returns so the hook order is
...
function AttachedTerminal({
	...
	// Pane errors (PTY crash, pane error) surface through the generic banner.
	...
	return (
		<div className="terminal-surface flex h-full min-h-0 flex-col" data-testid="session-terminal">
			...
			<div className="relative min-h-0 flex-1 pl-2">
				...
				{showReplayCover && <ReplayCover message={undefined} />}
				{banner && (
...
function ReplayCover({ message }: { message?: string }) {
	return (
		// xterm remains live underneath. Replay stays deliberately silent to
		// avoid covering settled output.
		<div
