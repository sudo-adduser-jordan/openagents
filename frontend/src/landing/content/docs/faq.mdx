...
<Accordions expandAll>
	...
	<Accordion title="Do I have to use a specific agent?">
		No. AO includes adapters for more than twenty harnesses, including Codex, Cursor, Aider, OpenCode, Droid, Kimi Code, and others. Native Chat currently supports installed Codex, OpenCode, and Droid; other harnesses use Terminal UI.
	</Accordion>
	...
	<Accordion title="Can Chat and Terminal UI run at the same time for one session?">
		No. A session commits to one controller at a time. Compatible Codex and OpenCode sessions can hand off between interfaces, but AO drains or interrupts the old controller before activating the new one.
	</Accordion>
