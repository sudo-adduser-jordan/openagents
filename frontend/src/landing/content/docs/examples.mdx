...
3. Select an installed Chat-capable harness such as Codex or OpenCode.
...
## Switch Codex Or OpenCode To Chat
...
```bash
open-agents spawn --project my-project --agent codex --name "Finish PR 142" --claim-pr 142
```
...
Open a session with a linked pull request, go to **Reviews**, select a configured reviewer harness, and start a review. Reviewer runs are separate from worker harness configuration. Current reviewer harnesses include Codex and OpenCode.
