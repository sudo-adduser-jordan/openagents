...
Compatible Codex and OpenCode sessions expose **Open Chat** or **Open Terminal UI**. An idle session can switch directly. If a turn is busy, AO asks whether to finish and drain it or stop and interrupt it before replacing the controller. The AO session and worktree remain the same.
