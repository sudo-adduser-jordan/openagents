...
Open Agents is a local, single-user system built around a long-running Go daemon. The Electron/React desktop app, optional CLI, and Expo mobile app are thin clients over the daemon's HTTP, SSE, and terminal interfaces.
...
For compatible Codex and OpenCode sessions, an interface transition can move the same provider conversation between TUI and Chat. Open Agents drains or interrupts the old controller before committing the replacement, so both controllers are never live at once. The session and worktree do not change.
...
All application state remains under `~/.open-agents`, including the run file, managed worktrees, Electron data, and mobile configuration. `OPEN_AGENTS_DATA_DIR` and `OPEN_AGENTS_RUN_FILE` provide advanced overrides for daemon storage and discovery.
