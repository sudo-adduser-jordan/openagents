---
title: Changelog
description: Where to find current Open Agents desktop and mobile release notes.
---

Open Agents now ships continuously through GitHub Releases. The release page is the source of truth for desktop versions, supported artifacts, changes, and upgrade notes.

The desktop app checks for updates automatically. Open **Settings → Updates** to see the installed version or check manually. The optional CLI also reports its own version with `open-agents version`.

- [Current desktop releases](https://github.com/sudo-adduser-jordan/open-agents/releases)
- [Current source history](https://github.com/sudo-adduser-jordan/open-agents/commits/main)
...
- Launched the website and docs at [orchestrator.inc](https://orchestrator.inc), with setup guides, API reference material, and plugin documentation in one place.
- Added Cursor as a first-class agent alongside Codex, Aider, OpenCode, and Hermes, using the same worktrees and PR workflow.
...
- Hardened the CLI with install-aware `open-agents update`, startup update notifications, doctor version checks, JSON output for `open-agents session ls`, config error handling, version mismatch fixes, and default filtering for terminated sessions.
- Moved terminals and sessions onto a single multiplexed WebSocket connection, reducing connection churn and resource leaks.
- Added prompt-driven spawn: `open-agents spawn` can now create sessions from freeform tasks without requiring a tracker issue.
...
- Renamed all packages from `@composio` to `@openagents` for a cleaner ecosystem identity.
