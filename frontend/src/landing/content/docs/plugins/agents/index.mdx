...
Every other harness is Terminal UI-only. Choose the initial interface with `ao spawn --mode chat|tui`. Compatible Codex and OpenCode sessions can hand off between Chat and Terminal UI while retaining the AO session and worktree.

## Choosing and configuring a harness

```bash
ao project set-config my-app --worker-agent codex
ao spawn --project my-app --name auth-fix --agent opencode --mode chat
```
