...
Choose the default for new sessions in project settings. You can also select it explicitly from the new-session flow or with `ao spawn --mode chat|tui`. Compatible Codex and OpenCode sessions can switch interfaces later without changing the AO session or worktree.
