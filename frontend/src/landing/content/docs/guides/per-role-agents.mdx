...
```bash
open-agents spawn --project my-project --kind orchestrator \
...
- Use **Chat** for structured messages, provider activity, approvals, plans, changed files, usage, compaction, and rollback. Current native drivers cover Codex, OpenCode, and Droid.
- Use **Terminal UI** when you want the agent's native interface or a harness without native Chat support.

Compatible Codex and OpenCode sessions expose **Open Chat** or **Open Terminal UI**. Idle sessions switch directly. When work is active, Open Agents asks whether to finish and drain it or stop and interrupt it before switching controllers.
