...
//
// (2) is the defensive backstop for issue #367: a standalone `open-agents daemon` may be
...
// The daemon's default bind port (backend/internal/config). OPEN_AGENTS_PORT overrides it.
export const DEFAULT_DAEMON_PORT = 3001;
// The `service` field every genuine Open Agents daemon stamps on its health payloads.
export const DAEMON_SERVICE_NAME = "open-agents-daemon";
...
export type DaemonProbe = {
	...
	/** Stable outer .AppImage path (OPEN_AGENTS_APPIMAGE), present only when the daemon's launcher runs under AppImage. */
	appImagePath?: string;
};
...
/**
 * The port a freshly spawned daemon is expected to bind: OPEN_AGENTS_PORT when set and
...
export function expectedDaemonPort(env: Record<string, string | undefined>): number {
	const configured = env.OPEN_AGENTS_PORT ? Number(env.OPEN_AGENTS_PORT) : NaN;
