...
const outPath = join(outDir, process.platform === "win32" ? "open-agents.exe" : "open-agents");
...
const buildOutPath = isWindowsDev ? join(windowsDevOutDir, "open-agents.exe") : outPath;
...
const result = spawnSync("go", ["build", "-o", buildOutPath, "./cmd/open-agents"], {
	cwd: backendRoot,
	stdio: "inherit",
	windowsHide: true,
});
