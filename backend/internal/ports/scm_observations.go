...
type SCMObservation struct {
	...
	Host string
	// Repo is the full repository name shown to Open Agents users, usually "owner/name".
	Repo string
	...
	// Mergeability contains Open Agents's mergeability verdict and blockers.
...
type SCMPRObservation struct {
	...
	Number int
	// State is Open Agents's normalized PR state: draft, open, merged, or closed.
	State string
...
type SCMCIObservation struct {
	// Summary is Open Agents's normalized aggregate CI state: unknown, pending, passing, or failing.
	Summary string
...
type SCMCheckObservation struct {
	...
	Name string
	// Status is Open Agents's normalized check status.
	Status string
...
type SCMReviewObservation struct {
	// Decision is Open Agents's normalized review decision.
	Decision string
...
type SCMReviewSummaryObservation struct {
	...
	Author string
	// State is Open Agents's normalized review decision for this review.
	State string
...
type SCMMergeabilityObservation struct {
	// State is Open Agents's normalized mergeability state.
	State string
