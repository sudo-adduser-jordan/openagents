...
// #2775 case (session open-agents-78) had a registration and DB row
...
// for the real #2775 case: session open-agents-78 had its worktree
// registered on a child branch (open-agents/open-agents-78/gh-pages-landing),
// not the root branch Open Agents would pass as cfg.Branch. When the directory is
...
func TestResolveWorktreeRefsInfersRepoDefaultBranchWhenUnset(t *testing.T) {
	...
	refs, err := ws.resolveWorktreeRefs(context.Background(), context.Background(), "/repo/child", "open-agents/work", "")
	if err != nil {
