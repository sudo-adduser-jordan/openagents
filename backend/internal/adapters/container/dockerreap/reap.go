...
// docker CLI. It force-removes containers labeled open-agents.session=<id>, skipping
// any also labeled open-agents.spare=true — the opt-out for deliberately shared
...
// `--label open-agents.session=$OPEN_AGENTS_SESSION_ID` so Open Agents can identify containers it owns.
const SessionLabel = "open-agents.session"

// SpareLabel opts a container out of reaping even though it carries
// SessionLabel — set `--label open-agents.spare=true` on a deliberately shared
// container (a shared postgres, a registry) that must survive the session
// that happened to start it.
const SpareLabel = "open-agents.spare"
...
// open-agents.session=<id> that is not also labeled open-agents.spare=true.
//
...
// never a best-guess sweep. A missing docker binary is NOT an error: most Open Agents
...
// present daemon. Most Open Agents installs run without Docker at all; that must
