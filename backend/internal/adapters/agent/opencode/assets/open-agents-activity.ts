// open-agents: managed opencode activity plugin (do not edit)
//
// It maps opencode's native lifecycle events onto Open Agents's normalized
// activity events:
//   session.created                       -> `open-agents hooks opencode session-start`
//   message.updated / message.part.updated -> `open-agents hooks opencode user-prompt-submit`
//   tool.execute.before / tool.execute.after -> `open-agents hooks opencode active`
//   session.status (status.type == idle)   -> `open-agents hooks opencode stop`
//   permission.asked / question.asked      -> `open-agents hooks opencode permission-blocked`
//   permission/question reply or rejection -> `open-agents hooks opencode active`
//
// The opencode-native session id (and prompt/model where known) is piped to the
// hook command as JSON on stdin, run with cwd set to the worktree so Open Agents can
// correlate the opencode session to its Open Agents session. Every invocation is
// best-effort and must never crash the user's opencode session: a missing `open-agents`
// binary is a guarded no-op (`command -v open-agents`), and spawn exceptions, non-zero
...
export const openAgentsActivity: Plugin = async ({ directory, client }) => {
  ...
  // Open Agents fences each provider generation with OPEN_AGENTS_RUNTIME_LAUNCH_ID; carry it in
  // the payload so hooks work even when child-process env inheritance is
  // trimmed by the host runtime.
  const launchID: string = (process.env.OPEN_AGENTS_RUNTIME_LAUNCH_ID ?? "").trim()
  const messageStore = new Map<string, any>()

  // Wrap in `sh -c` with a guard so a missing `open-agents` binary is a silent no-op
  // (exit 0) rather than a per-event error in the user's session.
  function hookCmd(hookName: string): string[] {
    return ["sh", "-c", `if ! command -v open-agents >/dev/null 2>&1; then exit 0; fi; exec open-agents hooks opencode ${hookName}`]
  }
  ...
  function logHookFailure(hookName: string, detail: string) {
    try {
      void client?.app
        ?.log?.({ body: { service: "open-agents-activity", level: "error", message: `hook ${hookName} failed: ${detail}` } })
        ?.catch?.(() => {})
    } catch {
  ...
  //      session-start, so Open Agents would see the prompt before the session is
  ...
  //
  // A non-zero exit (the guard makes a missing `open-agents` exit 0, so this is a real
  // `open-agents hooks` failure) or a spawn exception is logged with its stderr and never
  ...
  function callHookSync(hookName: string, payload: Record<string, unknown>) {
    try {
      const result = Bun.spawnSync(hookCmd(hookName), {
        cwd: directory,
        env: { ...process.env, OPEN_AGENTS_RUNTIME_LAUNCH_ID: launchID },
  ...
  // prompt never reaches Open Agents and title-from-prompt metadata breaks. Therefore: an
      ...
      try {
        switch (event.type) {
          ...
          case "session.status": {
            ...
            // Open Agents's "stop" hook means "the current turn is idle/finished", not
