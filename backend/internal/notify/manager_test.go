...
func TestHubProjectFilter(t *testing.T) {
	...
	defer unsub()
	_ = hub.Publish(context.Background(), domain.NotificationEvent{Record: domain.NotificationRecord{ID: "skip", ProjectID: "open-agents"}})
...
// Resolution is how a notification leaves the unresolved list: Open Agents observed the
