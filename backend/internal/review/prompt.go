...
// command carrying the ids) lives in the prompt, so it is also what Open Agents injects
...
1. For each PR below, post a separate review on that pull request and capture its id in one call. Post with `+"`gh api`"+` rather than `+"`gh pr review`"+`: it is the only way to attach inline comments, and its response carries the created review's id, so Open Agents can tell the worker exactly which review to address. Send the review as a JSON body so the inline comments form a proper array of objects:
   ...
   - Always use "event": "COMMENT": reviews are posted from the PR author's own account, and GitHub rejects both APPROVE and REQUEST_CHANGES on your own PR. State in the body whether you are requesting changes or approving; the machine-readable verdict goes to Open Agents in step 2.
   - The printed number is the review id. If the call fails on the provider, leave the id empty.
2. After every PR has its own GitHub review from step 1, record Open Agents's bookkeeping for those already-posted reviews using one command. Pass JSON on stdin so nothing is ever written into the worktree (a file there could be committed onto the worker's branch). Include one object per PR/run from the queue:

    printf '%%s' '{ "reviews": [ { "runId": "<run-id>", "verdict": "<approved|changes_requested>", "githubReviewId": "<id-from-step-1-or-empty>", "body": "<your full review markdown>" } ] }' | open-agents review submit --session %s --reviews -
...
func reviewQueueText(spec LaunchSpec) string {
	...
	fmt.Fprintf(&b, "\nOpen Agents created %d review tasks for this worker session. Review every queued PR, then submit all results together.\n\nReview task queue:\n", len(spec.ReviewQueue))
