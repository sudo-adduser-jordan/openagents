...
// writeMarker writes a ~/.open-agents/app-state.json marker pointing at appPath into the
// configured state dir (OPEN_AGENTS_RUN_FILE's directory).
...
// launched leaves the marker pointing at itself, and `open-agents start` would reopen
...
func TestAssetName_PerOS(t *testing.T) {
	...
	switch runtime.GOOS {
	case "darwin":
		if got != "open-agents-darwin-arm64.zip" && got != "open-agents-darwin-x64.zip" {
			t.Fatalf("darwin assetName = %q", got)
		}
	case "windows":
		if got != "open-agents-win32-x64.exe" {
			t.Fatalf("windows assetName = %q, want open-agents-win32-x64.exe", got)
		}
	case "linux":
		if got != "open-agents-linux-x64.AppImage" {
			t.Fatalf("linux assetName = %q, want open-agents-linux-x64.AppImage", got)
		}
	}
}
...
func TestWindowsInstalledExe(t *testing.T) {
	...
	want := filepath.Join("C:\\Users\\me\\AppData\\Local", "Programs", "Open Agents", "open-agents.exe")
	if got != want {
...
func TestIsUsableBundle_RegularFileVsDir(t *testing.T) {
	dir := t.TempDir()
	file := filepath.Join(dir, "open-agents.AppImage")
	if err := os.WriteFile(file, []byte("x"), 0o755); err != nil {
		t.Fatal(err)
	}
	subdir := filepath.Join(dir, "Open Agents.app")
...
func TestDownloadURLUsesReleaseRepo(t *testing.T) {
	...
	got := downloadURL("open-agents-darwin-arm64.zip")
	want := "https://github.com/owner/repo/releases/latest/download/open-agents-darwin-arm64.zip"
	if got != want {
...
func TestRegisterLinuxProtocolHandler(t *testing.T) {
	...
	appPath := "/tmp/Open Agents 100%.AppImage"
	...
	if !strings.Contains(content, `Exec="/tmp/Open Agents 100%%.AppImage" %u`) {
		t.Fatalf("desktop entry does not safely target AppImage:\n%s", content)
	}
	if !strings.Contains(content, "MimeType=x-scheme-handler/open-agents;") {
		t.Fatalf("desktop entry does not register open-agents:\n%s", content)
	}
	wantCommands := [][]string{
		{"xdg-mime", "default", linuxDesktopEntryName, "x-scheme-handler/open-agents"},
		{"xdg-mime", "query", "default", "x-scheme-handler/open-agents"},
	}
...
func TestOpenApp_ArgConstruction(t *testing.T) {
	...
	opened, err := c.openApp(context.Background(), "/Applications/Open Agents.app")
	if err != nil {
		t.Fatal(err)
	}
	if !opened {
	...
	wantArgs := []string{"/Applications/Open Agents.app", "--args", "--installed-via=npm-bootstrap"}
...
func TestOpenApp_DetachedSpawnOnWinLinux(t *testing.T) {
	...
	appPath := "/some/open-agents.AppImage"
...
func TestDownload_NonTTYProgress(t *testing.T) {
	...
	if err := c.download(context.Background(), &buf, srv.URL, "open-agents.zip", dst); err != nil {
	...
	if !strings.Contains(out, "Downloading Open Agents (open-agents.zip, ") {
...
func TestDownload_NoContentLengthOmitsSize(t *testing.T) {
	...
	if !strings.Contains(out, "Downloading Open Agents (asset.bin) from") {
