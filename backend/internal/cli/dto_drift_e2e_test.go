...
// dto_drift_e2e_test.go is the DTO-drift guard for the `open-agents spawn` and
// `open-agents project add` commands. The CLI defines its OWN request structs
...
// server purely via OPEN_AGENTS_RUN_FILE + the run-file port, so this is a genuine
...
func startDriftTestDaemon(t *testing.T, sessions controllers.SessionService, projects projectsvc.Manager) {
	...
	t.Setenv("OPEN_AGENTS_RUN_FILE", rfPath)
