...
const (
	...
	ConfigPath              = ".open-agents/launch.json"
...
const (
	...
	// StateStopping indicates that Open Agents is terminating the preview process.
...
// Configuration is one named server entry in .open-agents/launch.json. ${PORT} is
// expanded in runtimeArgs, url, and env values after Open Agents chooses the port.
...
type serverRun struct {
	status Status
	cmd    *exec.Cmd
	// startTime pins cmd's PID to the process Open Agents launched (kernel start time,
...
// Manager supervises at most one managed preview server per Open Agents session.
type Manager struct {
	log    *slog.Logger
	client *http.Client
...
// Stop terminates the exact process tree Open Agents launched for the session.
...
func (m *Manager) stop(ctx context.Context, sessionID domain.SessionID) (Status, error) {
	...
	case <-done:
		...
		// Open Agents's preview, so nothing is killed: a leaked preview process is
...
func (m *Manager) waitForExit(ctx context.Context, sessionID domain.SessionID, run *serverRun) {
	...
	// provably Open Agents's. No escalation happens here: descendants the dead root
...
func (m *Manager) failAndStop(
	...
	if cmd != nil {
		...
		select {
		case <-run.done:
			// Reaped: the PID is no longer provably Open Agents's, nothing to escalate.
...
func previewEnvironment(
	...
	env = append(env,
		...
		"OPEN_AGENTS_PREVIEW_PORT="+strconv.Itoa(port),
		"OPEN_AGENTS_SESSION_ID="+string(sessionID),
	)
	return env
}
