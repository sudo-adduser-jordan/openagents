...
func TestNotificationStore_ListUnreadNewestFirstAcrossProjects(t *testing.T) {
	...
	openAgents, _ := s.CreateSession(ctx, sampleRecord("open-agents"))
	base := time.Now().UTC().Truncate(time.Second)
	for _, rec := range []domain.NotificationRecord{
		...
		{ID: "other", SessionID: openAgents.ID, ProjectID: openAgents.ProjectID, Type: domain.NotificationNeedsInput, Title: "other", Status: domain.NotificationUnread, CreatedAt: base.Add(2 * time.Minute)},
	} {
		if _, inserted, err := s.CreateNotification(ctx, rec); err != nil || !inserted {
			t.Fatalf("insert %s inserted=%v err=%v", rec.ID, inserted, err)
		}
