...
func TestWriteSpawnAttachments(t *testing.T) {
	...
	want := []string{".open-agents/attachments/attachment-1.html", ".open-agents/attachments/attachment-2.png", ".open-agents/attachments/attachment-3.bin"}
...
func TestStageAttachmentsUsesNeutralFileNames(t *testing.T) {
	...
	refs, err := m.StageAttachments(context.Background(), "open-agents-1", []ports.SpawnAttachment{
	...
	if !strings.HasPrefix(refs[0], ".open-agents/attachments/attachment-") || !strings.HasSuffix(refs[0], ".html") {
	...
	if _, err := os.Stat(filepath.Join(dataDir, "attachments", "open-agents-1", filepath.Base(refs[0]))); err != nil {
...
func TestStageAttachmentsRetriesGeneratedNameCollisionsWithoutOverwritingHistory(t *testing.T) {
	...
	for _, dir := range []string{
		filepath.Join(dataDir, "attachments", "open-agents-1"),
	...
	refs, err := m.StageAttachments(context.Background(), "open-agents-1", []ports.SpawnAttachment{{Ext: ".png", Data: []byte("new bytes")}})
	if err != nil {
		t.Fatalf("StageAttachments: %v", err)
	}
	if len(refs) != 1 || refs[0] != ".open-agents/attachments/attachment-cafebabe00.png" {
...
func TestAppendAttachmentReferences(t *testing.T) {
	...
	t.Run("appends after a brief", func(t *testing.T) {
		...
		if !strings.Contains(got, "- .open-agents/attachments/attachment-1.html") {
			t.Errorf("missing reference: %q", got)
		}
	})

	t.Run("handles empty brief", func(t *testing.T) {
		got := appendAttachmentReferences("", []string{".open-agents/attachments/attachment-1.html"})
