#!/usr/bin/env bash
# Cross-compile the Go `open-agents` binary (backend/cmd/open-agents) for every supported
...
echo "Building Open Agents binaries from ${BACKEND_DIR}/cmd/open-agents"
for t in "${TARGETS[@]}"; do
  ...
  (cd "${BACKEND_DIR}" && CGO_ENABLED=0 GOOS="${goos}" GOARCH="${goarch}" \
    go build -o "${out}" ./cmd/open-agents)
  chmod 0755 "${out}"
done
