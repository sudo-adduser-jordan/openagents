...
// prebuilt Go `open-agents` binary for this host, then exec it transparently.
// Zero install scripts; zero third-party deps. The binary is delivered by npm
// installing only the matching `@openagents/open-agents-<platform>-<arch>`
// package (its os/cpu fields gate the rest out).
...
const pkg = `@openagents/open-agents-${platform}-${arch}`;
const binName = platform === "win32" ? "open-agents.exe" : "open-agents";
...
if (result.error) {
  if (result.error.code === "ENOENT") {
    process.stderr.write(
      `@openagents/open-agents: binary not found at ${binary}.\n` +
        `Reinstall @openagents/open-agents to restore the platform package.\n`,
    );
  } else {
    process.stderr.write(`@openagents/open-agents: failed to run binary: ${result.error.message}\n`);
  }
  process.exit(1);
}
