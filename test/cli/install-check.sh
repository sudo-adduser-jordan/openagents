#!/usr/bin/env bash
#
# Fresh-machine install check. The Dockerfile installs `open-agents` on PATH in a clean
...
OPEN_AGENTS_BIN="${OPEN_AGENTS_BIN:-open-agents}"
tmp="$(mktemp -d)"
...
echo "open-agents binary : $(command -v "$OPEN_AGENTS_BIN")"
"$OPEN_AGENTS_BIN" version            >/dev/null || fail "version"
"$OPEN_AGENTS_BIN" doctor             >/dev/null || fail "doctor"

# `open-agents start` is now the desktop-app launcher: it resolves an installed app or
...
# with a clear `open-agents start:` error (an unreachable/404 download on amd64, or an
...
if err="$("$OPEN_AGENTS_BIN" start 2>&1)"; then
  fail "start unexpectedly succeeded on a fresh machine with no installed app"
fi
echo "$err" | grep -qiE "download|open-agents start:" || fail "start did not fail with a clear error; got: $err"
